...
TairMaxTiff_folder_extrem = r'D:\Projects\Postdoc urban greening\Landsat_scale_data_codes\NDVI\\'
...
for i in cities_ID[1:]:
    ...
    try:
        tairMax_extreme = tf.imread(TairMaxTiff_folder_extrem + 'Landsat_ndvi_' + str(i) + '.0.tif')
...
CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID','ndvi_tree', 'ndvi_grass', 'ndvi_cropland', 'ndvi_build', 'r']
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\ndvi_landcover_types.csv', index=False)
