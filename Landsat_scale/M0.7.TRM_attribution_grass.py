...
LE = df['ET_tree']*10000/(3600*24)*(df['LE_ratio']*0.85)
...
LE_bu = df['ET_build']*10000/(3600*24)*(df['LE_ratio']*0.5)
...
dra = ra-ra_bu
dTs_ra = (ru0*rhoa*Lv*VPD/(ra+rs)**2 * 1/(1+f_trm) - (Rn-G-LE)*ru0/(1+f_trm)**2*(ftrm_ra)) * dra + 0.2
...
dTs_rs = (ru0*rhoa*Lv*VPD/(ra+rs)**2 * 1/(1+f_trm) - (Rn-G-LE)*ru0/(1+f_trm)**2*(ftrm_rs)) * drs + 2

term3 = dTs_rs
# plt.figure(); plt.plot(H-H_bu,df['ts_tree_lds'] - df['ts_build_lds'],'o')
...
df.to_csv(current_dir + '/2_Output/Cooling_Efficiency/grass_partition.csv', index=False)

import seaborn as sns

plt.figure();
plt.scatter(x[~mask], y[~mask],7,c='k')
sns.kdeplot(x=x[~mask], y=y[~mask],cmap="RdYlBu_r", fill=True,thresh=0.05,alpha = 0.8)
