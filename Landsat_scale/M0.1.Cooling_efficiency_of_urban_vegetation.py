...
TairMaxTiff_folder = r'D:\Projects\Postdoc urban greening\Landsat_scale_data_codes\LST\\'
...
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\CE_Landsat_yearly_Ts_v2.1.csv', index=False)
plt.figure(); plt.hist(IQR_filter(CE.iloc[:, -1]), 20, ec='black')

np.sum(CE.iloc[:, 3] > 0) / np.sum(~np.isnan(CE.iloc[:, 1]))
