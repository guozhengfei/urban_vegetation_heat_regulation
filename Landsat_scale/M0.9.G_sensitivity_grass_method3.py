...
# G_ = Rn*(0.32-0.21*df['ndvi_tree'].values)*0.9 # method 2
G = Rn*df['ts_tree_lds']*(0.0038+0.00074*alb)*(1-0.98*df['ndvi_tree'].values**4) # method 3
...
G_bu = Rn*df['ts_build_lds']*(0.0038+0.00074*alb)*(1-0.98*df['ndvi_build'].values**4) # method 3
...
df.loc[:,['ra_induced_dT']] = df.loc[:,['ra_induced_dT']]-0.5
...
ax.set_ylim([-3.5,1.5])
figToPath = current_dir + '/4_Figures/FigS_G_sensitivity_grass_method3'
fig.tight_layout()
fig.savefig(figToPath, dpi=600)
# plt.close(fig)
