...
imageC = ee.ImageCollection('MODIS/061/MOD16A2GF').select('ET').filterDate('2015-01-01', '2023-01-01').toBands()
...
np.save(current_dir+'/2_Output/Cooling_Efficiency/ET_tree.npy',ET_tree)
ET_grass = ET_array[3740:3740*2,:]
np.save(current_dir+'/2_Output/Cooling_Efficiency/ET_grass.npy',ET_grass)
ET_crop = ET_array[3740*2:,:]
np.save(current_dir+'/2_Output/Cooling_Efficiency/ET_crop.npy',ET_crop)

names = list(values_dict.keys())
names2 = names+['lon','lat','ID']
ET_tree = np.load(current_dir+'/2_Output/Cooling_Efficiency/ET_tree.npy', allow_pickle=True)
...
df_ET_tree.to_csv(current_dir+'/2_Output/Cooling_Efficiency/ET_tree.csv', index=False)

ET_grass2 = np.concatenate((ET_grass,grass_xy_rsp),axis=1)
df_ET_grass = pd.DataFrame(ET_grass2)
df_ET_grass.columns = names2
df_ET_grass.to_csv(current_dir+'/2_Output/Cooling_Efficiency/ET_grass.csv', index=False)

ET_crop2 = np.concatenate((ET_crop,crop_xy_rsp),axis=1)
df_ET_crop = pd.DataFrame(ET_crop2)
df_ET_crop.columns = names2
df_ET_crop.to_csv(current_dir+'/2_Output/Cooling_Efficiency/ET_crop.csv', index=False)
...
ID_values2.remove(689); ID_values2.remove(703);
