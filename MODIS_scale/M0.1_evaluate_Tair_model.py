...
for i in cities_ID:
    ...
    buildC = tf.imread(buildC_folder + 'buildingC_' + str(i) + '.0.tif') + tf.imread(bareC_folder + 'bareC_' + str(i) + '.0.tif')
    ...
    popC = tf.imread(pop_folder + 'pop_' + str(i) + '.0.tif')[:,:,-1]
    ...
    dem = Dem.reshape(-1)
    pop = popC.reshape(-1)
    mask = np.isnan(tree) | np.isnan(grass) | np.isnan(water) | np.isnan(tair) | np.isnan(dem) | np.isnan(pop)

    # plt.figure(); plt.hist(water[~mask], bins = 50)
    # Prepare the data
    X = np.array([tree[~mask], grass[~mask],  crop[~mask], build[~mask],  water[~mask], dem[~mask], pop[~mask], tair[~mask]]).T # shrub[~mask], build[~mask],,
    X = pd.DataFrame(X)
    X.columns = ['tree', 'grass',  'crop', 'building',  'water', 'dem', 'pop', 'tair'] #'shrub', 'building',
    folds = 100
...
scipy.io.savemat(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\model_performance_2020_yearly_withpop.mat', {'R': R})
