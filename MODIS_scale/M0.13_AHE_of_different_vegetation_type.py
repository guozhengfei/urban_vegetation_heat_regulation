...
aheTiff_folder = current_dir+'/1_Input/AHE_100m/'
...
for i in cities_ID[1:]:
    ...
    try:
        AHE = tf.imread(aheTiff_folder + 'AHE_' + str(i) + '.0.tif')
    except FileNotFoundError:
        continue
    # if np.isnan(np.nanmean(AHE)): continue
    tairHot = AHE
    ...
    ts_tree = X['tair'].loc[X['tree']>=np.percentile(X['tree'],98)].mean()
    ts_grass = X['tair'].loc[X['grass'] >= np.percentile(X['grass'],98)].mean()
    ts_crop = X['tair'].loc[X['crop'] >=np.percentile(X['crop'],98)].mean()
    ts_build = X['tair'].loc[X['impervious'] >= np.percentile(X['crop'],98)].mean()
    ...
    ce = [ts_tree, ts_grass, ts_crop, ts_build] # r
    CE.append([i] + ce)
    print(i)

CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID', 'ahe_tree','ahe_grass', 'ahe_cropland', 'ahe_build']
CE.to_csv(current_dir+'/2_Output/Cooling_Efficiency\AHE_Landsat_yearly_v1.csv', index=False)
