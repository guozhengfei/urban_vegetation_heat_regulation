...
for i in cities_ID[1:]:
    ...
    ts_tree = np.nanmean(predict1[0:2])
    ...
    predict1 = model.predict(X1.iloc[:, 0:-1]).tolist()
    # plt.figure(); plt.plot(predict1)
    ts_grass = np.nanmean(predict1[0:2])
    ...
    predict1 = model.predict(X1.iloc[:, 0:-1]).tolist()
    # plt.figure(); plt.plot(predict1)
    ts_crop = np.nanmean(predict1[0:2])
    ts_build = predict1[-1]
...
CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID', 'CEta_tree', 'ta_tree','CEta_grass', 'ta_grass', 'CEta_cropland', 'ta_cropland', 'ta_build', 'r']
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\CE_2020_yearly_v4.csv', index=False)
