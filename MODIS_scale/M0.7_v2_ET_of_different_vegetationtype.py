...
TairMaxTiff_folder = r'D:\Projects\Postdoc urban greening\Data\ET_canopy\\'
...
for i in cities_ID[1:]:
    try:
        tairMax = tf.imread(TairMaxTiff_folder + 'ET_' + str(i) + '.0.tif')
    ...
    #
    # # Fit the model
    # model = gam.fit(X.iloc[:, 0:-1], X.iloc[:, -1])
...
CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID', 'ET_tree', 'ET_grass', 'ET_cropland', 'ET_building' ,'r']
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\ET_2020_yearly_v5.csv', index=False)
