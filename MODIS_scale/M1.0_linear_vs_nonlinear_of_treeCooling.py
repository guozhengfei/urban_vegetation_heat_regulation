...
#D:\Projects\Postdoc urban greening\Data\TairMax\Monthly1000m
...
for i in cities_ID[1:]:
    ...
    X['grass'] = X['grass']+X['crop']
    X['grass'][X['grass']>1]=1
    slope_dem = st.linregress(X['dem'], X['tair']).slope
    X['tair2'] = X['tair'] - (X['dem'] - X['dem'].mean()) * slope_dem

    fig, ax = plt.subplots(1,2,figsize=(8, 3.2),sharey=True)
    colors = X[['tree', 'grass', 'impervious']].values.tolist()
    ax[0].scatter(X['tree'], X['tair2'], c=colors, s=10)
    ax[0].set_ylim([-3, 4])
    ax[1].scatter(X['grass'], X['tair2'], c=colors, s=10)
    figToPath = r'D:\Projects\Postdoc urban greening\Figures_main\cooling_effciency_explanation'
    fig.tight_layout()
    fig.savefig(figToPath, dpi=600)
    plt.close(fig)

    X1 = np.array([tree[~mask], grass[~mask], crop[~mask], build[~mask], water[~mask], dem[~mask], tair[~mask]]).T  #
    X1 = pd.DataFrame(X1)
    X1.columns = ['tree', 'grass', 'crop', 'impervious', 'water', 'dem', 'tair']
    X1['veg'] = X1['grass'] + X1['crop'] + X1['tree']
    X2 = X1[['veg','impervious', 'water', 'dem', 'tair']]
    ...
    model.fit(X2.iloc[:, 0:-1], X2.iloc[:, -1])
    predic = model.predict(X2.iloc[:, 0:-1]).tolist()
    r = st.linregress(X2.iloc[:, -1], predic).rvalue

    X3 = X2.iloc[0:101, :] * 1
    X3.iloc[:, 0] = np.linspace(1, 0, 101)
    X3.iloc[:, 1] = np.linspace(0, 1, 101)
    X1.iloc[:, 2] = 0
    X1.iloc[:, 3] = np.nanmean(dem)
    predict1 = model.predict(X3.iloc[:, 0:-1]).tolist()
    # plt.figure(), plt.plot(predict1)
    ce = np.nanmean(predict1[0:2]) -np.nanmean(predict1[-1])
...
CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID', 'CE_abundance', 'r']
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\CE_2020_yearly_Ts_vegetation_abundance.csv', index=False)
