...
TairMaxTiff_folder = r'D:\Projects\Postdoc urban greening\Data\Albedo\\'
...
for i in cities_ID[1:]:
    try:
        tairMax = tf.imread(TairMaxTiff_folder + 'albedo_' + str(i) + '.0.tif')
    ...
    predict = model.predict(X.iloc[:, 0:-1]).tolist()
    r = st.linregress(X.iloc[:, -1], predict).rvalue
    ###
    #plt.figure(); plt.plot(X.iloc[:, -1].values, predict,'o')
    X1 = X.iloc[0:101, :]*1
    ...
    # plt.figure(); plt.plot(data[:,3], values[:,3],'.')
...
CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID', 'Alb_tree', 'Alb_grass', 'Alb_cropland', 'r']
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\albedo_2020_yearly_v5.csv', index=False)
plt.figure(); plt.hist(IQR_filter(CE.iloc[:, 3]), 20, ec='black')

np.sum(CE.iloc[:, 2]<0)/np.sum(~np.isnan(CE.iloc[:, 2]))
