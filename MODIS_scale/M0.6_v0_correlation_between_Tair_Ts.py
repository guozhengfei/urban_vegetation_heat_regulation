...
TairMaxTiff_folder = r'D:\Projects\Postdoc urban greening\Data\TairMax\Monthly1000m\\'

TairMaxTiff_folder2 = r'D:\Projects\Postdoc urban greening\Data\TsMax\\'
...
for i in cities_ID[1:]:
    try:
        tairMax = tf.imread(TairMaxTiff_folder + 'TairMax_' + str(i) + '.0.tif')
        tairMax2 = tf.imread(TairMaxTiff_folder2 + 'Ts_' + str(i) + '.0.tif')
    ...
    tairHot2 = np.nanmean(tairMax2[:, :, 12 * 17:], axis=2)
    ...
    tair_bg2 = background_climate(tairHot2, Dem, urban)
    ...
    tair2 = (tairHot2 - tair_bg2).reshape(-1)

    dem = Dem.reshape(-1)
    mask = np.isnan(tree) | np.isnan(grass) | np.isnan(water) | np.isnan(tair) | np.isnan(dem) |  np.isnan(tair2)


    X = np.array([tree[~mask], grass[~mask], crop[~mask], build[~mask], water[~mask], dem[~mask], tair[~mask],tair2[~mask]]).T  # ,
    X = pd.DataFrame(X)
    X.columns = ['tree', 'grass', 'crop', 'impervious', 'water', 'dem', 'tair', 'ts']
    rval = st.linregress(X['tair'], X['ts']).rvalue
    # plt.figure(); plt.imshow((tairHot - tair_bg)*urban)
    # plt.figure(); plt.imshow((tairHot2 - tair_bg2) * urban)
...
R = pd.DataFrame(R)
R.columns = ['ID', 'r']
R.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\correlation_Ta_Ts.csv', index=False)
