...
CE = pd.read_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\CE_2020_yearly_v2.csv')
...
# plt.figure(); plt.plot(merged_df['MAT']-273.15, merged_df['tree'], 'o')
slope, intercept, r_squared, p_value = st.linregress(merged_df['MAT']-273.15, merged_df['tree'])[:4]
xd = np.linspace(merged_df['MAT'].min()-273.15,merged_df['MAT'].max()-273.15,100)
fit_line = slope * np.log(xd) + intercept

plt.figure()
plt.scatter(merged_df['MAT']-273.15, merged_df['tree'], label='Data')
plt.plot(xd, fit_line, 'r--', label='Fit Line')

# MAP vs. tree CEP
st.linregress(merged_df['MAP']*24, merged_df['tree'])
slope, intercept, r_squared, p_value = log_fit(merged_df['MAP']*24, merged_df['tree'])
xd = np.linspace(merged_df['MAP'].min()*24,merged_df['MAP'].max()*24,100)
fit_line = slope * np.log(xd) + intercept
plt.figure()
plt.scatter(merged_df['MAP']*24, merged_df['tree'], label='Data')
plt.plot(xd, fit_line, 'r-', label='Fit Line')
