...
for i in cities_ID:
    ...
    ce_tree = np.nanmean(predict1[0:5]) - np.nanmean(predict1[-5:-1]) # negative is cooling
    ...
    predict1 = model.predict(X1.iloc[:, 0:-1]).tolist()
    # plt.figure(), plt.plot(predict1)
    ce_grass = np.nanmean(predict1[0:5]) - np.nanmean(predict1[-5:-1]) # negative is cooling
    ...
    predict1 = model.predict(X1.iloc[:, 0:-1]).tolist()
    # plt.figure(), plt.plot(predict1)
    ce_crop = np.nanmean(predict1[0:5]) - np.nanmean(predict1[-5:-1]) # negative is cooling
...
CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID', 'tree', 'grass', 'cropland']
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\CE_2020_yearly_v2_night.csv', index=False)
