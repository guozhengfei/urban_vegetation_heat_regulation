...
def mean_slope(data,values):
    ...
    for m in range(4):
        ...
        data2 = values[:, m] + values.base_values
        mean_values = []
...
for i in cities_ID[1:]:
    try:
        tairMax = tf.imread(Ts_folder + 'Ts_' + str(i) + '.0.tif')
    ...
    Ts = np.nanmean(tairMax[:, :, 12 * 17:], axis=2)
    # plt.figure(); plt.imshow(LW)
    ratios = energy_ratios[energy_ratios['ID'] == i]

    SW = np.nanmean(tf.imread(sw_folder + 'sw_' + str(i) + '.0.tif')[:,:,-13:-1], axis=2)/(3600*24)*ratios['SW_ratio'].values

    LW = np.nanmean(tf.imread(lw_folder + 'lw_' + str(i) + '.0.tif')[:, :, -13:-1], axis=2)/(3600*24)*ratios['LW_ratio'].values
    ...
    buildC = tf.imread(impervious_folder + 'imperviousC_' + str(i) + '.0.tif') + tf.imread(bareC_folder + 'bareC_' + str(i) + '.0.tif')
    ...
    Dem = tf.imread(dem_folder + 'dem_' + str(i) + '.0.tif').astype(float)
    ...
    model = xgb.XGBRegressor(max_depth=2, min_child_weight=11, subsample=0.65, colsample_bytree=0.65, eta=0.15,tree_method='hist', max_delta_step=6, eval_metric='auc')  # lower max_depth, larger
    ...
    ce_tree = np.nanmean(predict1[0:2]) - np.nanmean(predict1[-1]) # negative is cooling
    ...
    predict1 = model.predict(X1.iloc[:, 0:-1]).tolist()
    # plt.figure(), plt.plot(predict1)
    ce_grass = np.nanmean(predict1[0:2]) - np.nanmean(predict1[-1]) # negative is cooling
    ...
    predict1 = model.predict(X1.iloc[:, 0:-1]).tolist()
    # plt.figure(), plt.plot(predict1)
    ce_crop = np.nanmean(predict1[0:2]) - np.nanmean(predict1[-1]) # negative is cooling

    ce_build = np.nanmean(predict1[-1])

    ce = [ce_tree, ce_grass, ce_crop, ce_build, r]
    CE.append([i] + ce)
    print(i)

CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID', 'Rn_tree', 'Rn_grass', 'Rn_cropland', 'Rn_building', 'r']
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\Rn_2020_yearly_v4.1.csv', index=False)
