...
# mat_data = scipy.io.loadmat(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\model_performance_2020_yearly_Ts.mat')
...
CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID', 'tree', 'grass', 'cropland','r']
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\CE_2020_yearly_v1_Ts.csv', index=False)
plt.figure(); plt.hist(IQR_filter(CE.iloc[:, 1]), 20, ec='black')

np.sum(CE.iloc[:, 3]<0)/np.sum(~np.isnan(CE.iloc[:, 3]))
