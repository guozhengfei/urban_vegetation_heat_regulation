...
TairMaxTiff_folder = r'D:\Projects\Postdoc urban greening\Data\ET_modis\\'
...
for i in cities_ID[1:]:
    try:
        tairMax = tf.imread(TairMaxTiff_folder + 'LE_' + str(i) + '.0.tif')
    except FileNotFoundError:
        continue
    if np.isnan(np.nanmean(tairMax)): continue

    tairHot = np.nanmean(tairMax, axis=2)
    ...
    buildC = tf.imread(bareC_folder + 'bareC_' + str(i) + '.0.tif')
    ...
    X1.iloc[:, 5] = np.nanmean(dem)*2
    predict1 = model.predict(X1.iloc[:, 0:-1]).tolist()
    # plt.figure(), plt.plot(predict1)
    ce_tree = np.nanmean(predict1[0:2])  # negative is cooling
    ...
    predict1 = model.predict(X1.iloc[:, 0:-1]).tolist()
    # plt.figure(), plt.plot(predict1)
    ce_grass = np.nanmean(predict1[0:2])  # negative is cooling
    ...
    predict1 = model.predict(X1.iloc[:, 0:-1]).tolist()
    # plt.figure(), plt.plot(predict1)
    ce_crop = np.nanmean(predict1[0:2])  # negative is cooling
    # ce_build = np.nanmean(predict1[-1])#
    ce_build = np.nanmin(tairHot)  #
...
CE = np.array(CE)
CE = pd.DataFrame(CE)
CE.columns = ['ID', 'ET_tree', 'ET_grass', 'ET_cropland','ET_build','r']
CE.to_csv(r'D:\Projects\Postdoc urban greening\Data\Cooling_Efficiency\ET_2020_yearly_all_modis.csv', index=False)
